...
import { Phone, MessageCircle, CalendarCheck, Clock, TrendingUp, ArrowRight } from "lucide-react";

export function KlinikProof() {
    const metrics = [
        { label: "Kaç çağrı", value: "342", icon: Phone },
        { label: "Kaç mesaj", value: "856", icon: MessageCircle },
        { label: "Kaç randevu talebi", value: "124", icon: CalendarCheck },
        { label: "Konuşma süresi", value: "48s", icon: Clock },
        { label: "Dönüşüm", value: "%36", icon: TrendingUp }
    ];
                ...
                <div className="max-w-4xl mx-auto">
                    ...
                    >
                        <h2 className="text-3xl md:text-4xl font-bold text-[#0F172A] mb-8">
                            Klinik Panelinde Neleri Görürsünüz?
                        </h2>
                    </motion.div>
                    ...
                    <div className="bg-[#0F172A] rounded-3xl p-8 md:p-12 shadow-2xl relative overflow-hidden border border-slate-800">
                        ...
                        >
                            <p className="text-lg md:text-xl text-slate-300 font-medium flex flex-col md:flex-row items-center justify-center gap-2 md:gap-4 flex-wrap">
                                <span>Hasta aradı</span>
                                <ArrowRight className="text-blue-400 hidden md:block" />
                                <span className="md:hidden text-blue-400">↓</span>
                                <span>FlowAsistan otomatik karşıladı</span>
                                <ArrowRight className="text-blue-400 hidden md:block" />
                                <span className="md:hidden text-blue-400">↓</span>
                                <span>randevu talebini aldı</span>
                                <ArrowRight className="text-blue-400 hidden md:block" />
                                <span className="md:hidden text-blue-400">↓</span>
                                <span className="text-white">siz panelden sonucu ve istatistikleri gördünüz.</span>
                            </p>
                        </motion.div>
                    </div>
                </div>
