import { klinikBlogPosts } from "@/data/klinik-blog";
import { MagicCard } from "@/components/ui/magic-card";
import { Link } from "@/i18n/routing";
import NextImage from "next/image";
import { ArrowRight, Calendar, Clock } from "lucide-react";

export default function KlinikBlogPage() {
    const highlightPost = klinikBlogPosts[0];
    const regularPosts = klinikBlogPosts.slice(1);

    return (
        <main className="min-h-screen bg-slate-900 text-slate-200 pb-24 pt-32">
            <div className="container mx-auto px-4 md:px-6 max-w-6xl">
                <div className="mb-16">
                    <h1 className="text-4xl md:text-5xl font-bold text-white mb-4 tracking-tight">Klinik Blog</h1>
                    <p className="text-slate-400 text-lg md:text-xl max-w-2xl">
                        Hasta iletişimi, randevu süreci ve 7/24 otomatik karşılama sistemleri üzerine en güncel vaka analizleri ve rehberler.
                    </p>
                </div>
                ...
                {highlightPost && (
                    <div className="mb-16">
                        <Link href={`/flowasistan/klinik/blog/${highlightPost.slug}`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {regularPosts.map((post) => (
                        <Link key={post.slug} href={`/flowasistan/klinik/blog/${post.slug}`} className="block h-full">
                ...
                <div className="mt-20">
                    <MagicCard className="p-8 md:p-12 text-center bg-slate-800/60 border-slate-700 relative overflow-hidden group">
                        ...
                        <div className="relative z-10">
                            <h3 className="text-2xl md:text-3xl font-bold text-white mb-4">Otomatik Karşılamayı Kliniğinizde Deneyin</h3>
                            <p className="text-slate-400 mb-8 max-w-xl mx-auto text-lg">
                                Süreçlerinizi nasıl otomatize edip çağrıları randevuya dönüştürebileceğimizi konuşmak için 30 dakikalık ücretsiz bir analiz görüşmesi planlayın.
                            </p>
                            <Link
                                href="/flowasistan/klinik#randevu"
