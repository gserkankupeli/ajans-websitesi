import { getAjansPostBySlug, ajansBlogPosts } from "@/data/ajans-blog";
...
export async function generateStaticParams() {
    return ajansBlogPosts.flatMap((post) => [
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    const post = getAjansPostBySlug(resolvedParams.slug);
    if (!post) return { title: "Yazı Bulunamadı | Ajans Blog" };
    return {
        title: `${post.title} | FlowAsistan Ajans Blog`,
        description: post.excerpt,
    };
}

export default async function AjansBlogPostPage({ params }: Props) {
    const resolvedParams = await params;
        ...
        <main className="min-h-screen bg-white">
            <div className="relative w-full h-[50vh] md:h-[60vh] bg-slate-900 border-b border-slate-800 flex items-center justify-center overflow-hidden">
                ...
                <div className="absolute inset-x-0 bottom-0 top-0 flex flex-col justify-end pb-16 md:pb-24 pt-32 container mx-auto px-4 md:px-6 max-w-4xl z-10">
                    <Link href="/flowasistan/ajans/blog" className="inline-flex items-center gap-2 text-blue-400 hover:text-blue-300 transition-colors mb-6 md:mb-10 text-sm font-medium w-fit">
            ...
            <div className="bg-slate-50 border-t border-gray-100 py-20">
                <div className="container mx-auto px-4 md:px-6 max-w-4xl text-center">
                    <h3 className="text-3xl font-bold text-[#0F172A] mb-4">Ajansınızın asistanı hazır</h3>
                    <p className="text-gray-600 mb-8 max-w-2xl mx-auto">Gelen talepler karşılanmadan, nitelendirmeden, panele düşmeden geçmesin. 30 dakikalık görüşmede birlikte netleştirelim.</p>
                    <Link href="/flowasistan/ajans#randevu" className="inline-flex h-12 items-center justify-center rounded-xl bg-[#2563EB] px-8 py-2 text-sm font-medium text-white shadow transition-colors hover:bg-[#1D4ED8]">
