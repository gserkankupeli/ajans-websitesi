...
import { emlakBlogPosts } from '@/data/emlak-blogs';
...
export default async function EmlakBlogListPage({ params }: PageProps) {
    const locale = (await params).locale;
    setRequestLocale(locale);

    const highlightPost = emlakBlogPosts[0];
    const regularPosts = emlakBlogPosts.slice(1);

    return (
        <div className="min-h-screen bg-slate-900 text-slate-200 pb-0 pt-32">
            <div className="container mx-auto px-4 md:px-6 max-w-6xl pb-24">

                {/* Header */}
                <div className="mb-16">
                    <h1 className="text-4xl md:text-5xl font-bold text-white mb-4 tracking-tight">Emlak İçin Blog</h1>
                    <p className="text-slate-400 text-lg md:text-xl max-w-2xl">
                        Niteliksiz müşteriyle zaman kaybını azaltma, hızlı dönüş ve takip düzeni üzerine kısa, net içerikler.
                    </p>
                </div>
                    ...
                    <div className="mb-16">
                        <Link href={`/emlak/blog/${highlightPost.slug}`}>
                            <MagicCard className="p-0 border-slate-800 bg-slate-800/50 hover:border-blue-500/30 group overflow-hidden flex flex-col md:flex-row items-stretch min-h-[400px]">
                                ...
                                <div className="w-full md:w-1/2 p-8 md:p-12 flex flex-col justify-center relative z-10 text-left">
