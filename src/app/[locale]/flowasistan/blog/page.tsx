import { flowAsistanBlogPosts } from "./data";
...
export default function FlowAsistanBlogList() {
    const highlightPost = flowAsistanBlogPosts[0];
    const regularPosts = flowAsistanBlogPosts.slice(1);

    return (
        <main className="min-h-screen bg-slate-900 text-slate-200 pb-0 pt-32">
            <FlowAsistanStickyHeader />
            <div className="container mx-auto px-4 md:px-6 max-w-6xl pb-24">
                <div className="mb-16">
                    <h1 className="text-4xl md:text-5xl font-bold text-white mb-4 tracking-tight">FlowAsistan Blog</h1>
                    <p className="text-slate-400 text-lg md:text-xl max-w-2xl">
                        İşletmeler için 7/24 otomatik karşılama ve tek panelden takip üzerine kısa, net içerikler.
                    </p>
                </div>
                ...
                {highlightPost && (
                    <div className="mb-16">
                        <Link href={`/flowasistan/blog/${highlightPost.slug}`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {regularPosts.map((post) => (
                        <Link key={post.slug} href={`/flowasistan/blog/${post.slug}`} className="block h-full">
